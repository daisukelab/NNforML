...
function ret = d_loss_by_d_model(model, data, wd_coefficient)
  ...
  d_hid = class_prob - data.targets; % size: <number of classes, i.e. 10> by <number of data cases>
  d_hid_weight = d_hid * hid_output' ./ size(data.targets, 2); % size <number of classes i.e. 10> by <number of hidden units>
  d_in  = model.hid_to_class' * d_hid .* (hid_output .* (1 - hid_output)) ./ size(data.targets, 2);
  d_in_weight  = d_in * data.inputs';

  ret.input_to_hid = d_in_weight + wd_coefficient*model.input_to_hid;
  ret.hid_to_class = d_hid_weight + wd_coefficient*model.hid_to_class;
end
